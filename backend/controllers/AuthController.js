import { getUserByEmailModel, getUserByIdModel } from "../models/UserModel.js";
...
export const Login = async (req, res) => {
  try {
    ...
    const responseGetUserByEmail = await getUserByEmailModel(email).catch(
      (message) => {
    ...
    if (
      Array.isArray(responseGetUserByEmail) &&
      responseGetUserByEmail.length === 0
    ) {
...
export const Me = async (req, res) => {
  try {
    ...
    const response = await getUserByIdModel(req.session.uuid_user).catch(
      (message) => {
        throw new Error(message);
      }
    );
...
export const logOut = async (req, res) => {
  req.session.destroy((err) => {
    if (err)
      return res.status(400).json({ errorMessage: "Tidak dapat logout!" });
