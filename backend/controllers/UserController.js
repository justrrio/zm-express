...
export const getUserByIdController = async (req, res) => {
  try {
    ...
    const response = await getUserByIdModel(uuid_user).catch((message) => {
      throw new Error(message);
    });
...
export const createUserController = async (req, res) => {
  const dataRaw = {
    ...
    konfirmasiPasswordUser: req.body.konfirmasiPasswordUser,
    noTlpUser: req.body.noTlpUser,
    roleUser: req.body.roleUser,
  };

  // Validasi nama
  const nameRegex = /^[a-zA-Z\s.'-]+$/;
  if (!nameRegex.test(dataRaw.namaUser)) {
    return res.status(400).json({ message: "Nama tidak valid!" });
  }

  // Validasi password dan confirm password
  if (dataRaw.passwordUser !== dataRaw.konfirmasiPasswordUser)
    return res
      .status(400)
      .json({ message: "Password dan Confirm Password tidak sesuai!" });
  const hashedPasswordUser = await argon2.hash(dataRaw.passwordUser);

  // Validasi format email
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(dataRaw.emailUser))
    return res.status(400).json({ message: "Email tidak valid!" });

  // Validasi format nomor telepon
  const phoneRegex = /^(\+62|0)8[1-9][0-9]{6,11}$/;
  if (!phoneRegex.test(dataRaw.noTlpUser))
    return res.status(400).json({ message: "Nomor telepon tidak valid!" });

  try {
    const dataProcessed = {
      namaUser: dataRaw.namaUser,
      emailUser: dataRaw.emailUser,
      hashedPasswordUser: hashedPasswordUser,
      noTlpUser: dataRaw.noTlpUser,
      roleUser: dataRaw.roleUser,
    };
    const response = await createUserModel(dataProcessed).catch((message) => {
      throw new Error(message);
    });
...
export const updateUserController = async (req, res) => {
  try {
    ...
    if (
      Array.isArray(responseGetUserById) &&
      responseGetUserById.length === 0
    ) {
      return res.status(400).json({ message: "Akun tidak ditemukan!" });
    }
