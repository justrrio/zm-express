...
export const deleteLayananController = async (req, res) => {
    try {
        ...
        const responseGetLayananById = await getLayananByIdModel(id_layanan).catch((message) => {
            throw new Error(message);
        });
        if (Array.isArray(responseGetLayananById) && responseGetLayananById.length === 0) {
            return res.status(400).json({message: "Layanan tidak ditemukan!"});
        }

        const responseDelete = await deleteLayananModel(id_layanan).catch((message) => {
            throw new Error(message);
        });
